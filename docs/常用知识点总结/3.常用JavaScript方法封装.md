...
```javascript
/**
 * 金额格式化：默认为金额为分，基数100，转换后以元为单位展示
 * @param v 金额(默认传进来的是分)
 * @param radix 转换金额的基数
 */
export const moneyRender = function (v: any, radix?: number, needCurrency = false) {
  const currency = '¥'
  if ((typeof v !== 'string' && typeof v !== 'number') || v === '')
    return needCurrency ? `${currency} 0.00` : 0;
  const num = radix || 100;
  const value = (typeof v === 'string' ? parseInt(v, 10) : v) / num;
  const money = (Math.round(value * 100) / 100).toFixed(2);
  return needCurrency && currency
    ? `${currency} ${money}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')
